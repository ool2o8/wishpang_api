...
class CreateUserViewset(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        return redirect("http://localhost/account/login")
...
class loginView(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if user is not None:
            login(request, user)
            return redirect("http://localhost/blog/post")
        else:
            return redirect("http://localhost/account/register")
...
class KakaoGetLogin(View):
    def get(self, request):
        ...
        REDIRECT_URI = 'http://localhost:80/account/kakao/login/callback'
    ...
    def get(self, request):
        ...
        if not Profile.objects.filter(kakao_id=user_info["id"]).exists():
            ...
            user_register_response = requests.post(
                'http://localhost/account/register', data=user_register_data)
